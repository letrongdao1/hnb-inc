...
const items = [
  {
    ...
    bgColor: "#0D0716",
    textColor: "#fff",
    links: [{ label: "Company", href: "/", ariaLabel: "About Company" }],
  },
  {
    label: "Projects",
    bgColor: "#170D27",
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 z-[2] flex h-[60px] items-center justify-between p-2 pl-[1.1rem]">
          ...
          <div className="logo-container order-1 flex items-center md:absolute md:top-1/2 md:left-1/2 md:order-none md:-translate-x-1/2 md:-translate-y-1/2">
            {logo && <Image src={logo} alt={logoAlt} className="logo h-[28px]" />}
          </div>

          <button
            type="button"
            className="card-nav-cta-button hidden h-full cursor-pointer items-center rounded-[calc(0.75rem-0.2rem)] border-0 font-medium transition-colors duration-300 md:flex"
            style={{ backgroundColor: buttonBgColor, color: buttonTextColor }}
          >
            <Link href={"/auth/login"} className="h-full p-4 flex items-center">Đăng nhập</Link>
          </button>
        </div>
